...
const PreviewTableCard: React.FC<{ form: any }> = ({ form }) => (
  <div className="glass-card animate-fade-in" style={{
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.25rem' }}>
      <div>
        <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: '1.25rem', color: '#fff' }}>
          {form.name || 'Your New Table'}
        </div>
        <div style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.45)', marginTop: 2, fontFamily: 'var(--font-mono)' }}>
          {form.smallBlind}/{form.bigBlind} · Custom
        </div>
      </div>
    ...
    <div style={{ marginBottom: '1.5rem' }}>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${form.maxPlayers}, 1fr)`, gap: '0.375rem', marginBottom: '0.5rem' }}>
        {Array.from({ length: form.maxPlayers }, (_, i) => (
          <div key={i} style={{ height: 5, borderRadius: 2.5, background: i === 0 ? 'var(--gold)' : 'rgba(255,255,255,0.08)' }} />
        ))}
      </div>
      <div style={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.4)', display: 'flex', justifyContent: 'space-between' }}>
        <span>1/{form.maxPlayers} players</span>
        {form.tokenGated && <span className="badge badge-purple" style={{ fontSize: '0.65rem' }}>Token Gated</span>}
      </div>
    </div>

    <div style={{
      background: 'rgba(0,0,0,0.2)', borderRadius: 12, padding: '1rem',
      border: '1px solid rgba(255,255,255,0.05)', marginBottom: '1.5rem',
    }}>
      <div style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>
        Table Rules
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', fontSize: '0.8125rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span className="subtle">Min Players</span>
          <span style={{ color: '#fff' }}>{form.minPlayers}</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span className="subtle">Time Bank</span>
          <span style={{ color: '#fff' }}>{form.timeBankSecs}s</span>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <span className="subtle">Betting</span>
          <span style={{ color: 'var(--arcium)' }}>No Limit</span>
        </div>
      </div>
    </div>

    <button className="btn btn-gold btn-sm" disabled style={{ width: '100%', opacity: 0.8 }}>
      Join Table
    </button>
  </div>
);
      ...
      <div style={{ maxWidth: 1080, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: '4rem', alignItems: 'start' }}>
          ...
          <div className="animate-fade-up delay-2" style={{ position: 'sticky', top: '2rem' }}>
            ...
            <PreviewTableCard form={form} />

            <div style={{ marginTop: '2rem' }} className="glass-panel">
              <div style={{ padding: '1rem', borderBottom: '1px solid var(--border)', display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <span style={{ color: 'var(--arcium)' }}>✦</span>
                <span style={{ fontWeight: 600, fontSize: '0.875rem' }}>Arcium Edge Computing</span>
              </div>
              <div style={{ padding: '1rem', fontSize: '0.8125rem', color: 'rgba(255,255,255,0.45)', lineHeight: 1.5 }}>
                Your table is powered by an Arcium MXE. All cards are encrypted and evaluated within an MPC cluster, ensuring zero-knowledge about player hands until the final showdown.
              </div>
            </div>
          </div>
        </div>
